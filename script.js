...
function initLoader(){
  ...
  function changePhrase() {
    ...
    if (currentIndex === 0) displayTime=3000; //Stop the sentence change at the fifth sentence, it goes to 0 because it comes after the increase of the index
...
function initSocialButton(){
    ...
    $("#Social-icon").on("click", () => {
      ...
      if (!$("#SocialButton").hasClass("magnetic")){
        $("#SocialButton").off("click");
        ...
        $("#SocialButton").addClass("magnetic")
        $("#SocialButton").css({ height: "60px", width: "60px "});
...
const section2Button = document.getElementById('changeSectionManual2');
...
const section1Button = document.getElementById('changeSectionManual1');
section1Button.addEventListener('click', function() {
    showAnim.play()
    setTimeout(() => { //prima mostra la navbar e poi cambia
      document.getElementById("radio-1").checked = true;
      fadeOutInTransition('content-1'); // Cambia con l'ID della tua prima sezione
      background1.style.opacity = 1;
        background2.style.opacity = 0;
        tab.style.color = "black"
    }, 300);
  ...
  function parallax() { 
    ...
    $('.parallax ').each(function(){ 
      if ($(this).is_on_screen()) {	
        ...
        var moveTop = (firstTop - winScrollTop) * 0.30; // speed
